...
thread_sweep(int tid, int iter, int lbound, int rbound)
{
        ...
        for (int row = 1; row < gs_size - 1; row++) {
                 ...
                 * to the left */
		while (tid != 0 && threads[tid-1].currentRow <= threads[tid].currentRow){
		}; 
...
thread_compute(void *_self)
{
	...
	lbound = (gs_size/4)*tid;
	rbound = lbound + (2048/4);
	if(tid==0){lbound++;}
	if(tid==3){rbound--;}
